...
import TwoTabPanel from '@/components/Sidebar/ui/TwoTabPanel';
...
export default function PersonalRecordPrompt({
  ...
  const isGemini = (url = "") => {
    ...
    return Gemini_DOMAINS.some(domain => url.includes(domain));
  };
  ...
  const clickEnterButton = async () => {
    ...
    try {
      ...
      if (!success) {
        throw new Error("sendPromptToGemini returned false");
      }
