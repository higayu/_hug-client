(async () => {
  ...
  try {
    ...
    console.log("[HUG WM] 一覧HTML:", listHtml);

    const parser = new DOMParser();
    const listDoc = parser.parseFromString(listHtml, "text/html");

    // body > div.contents > div.ibox > div:nth-child(7) > div > ul
    const targetUl = listDoc.querySelector(
      "body > div.contents > div.ibox > div:nth-child(7) > div > ul"
    );
    if (targetUl) {
      console.log("[HUG WM] 対象 ul が見つかりました", targetUl);
      const lis = [...targetUl.querySelectorAll("li")];
      console.log("[HUG WM] ul 内の li 数:", lis.length);
      lis.forEach((li, index) => {
        console.log(`[HUG WM] li[${index}]:`, li.textContent.trim(), li);
      });
    } else {
      console.warn("[HUG WM] 対象 ul が見つかりませんでした");
    }
