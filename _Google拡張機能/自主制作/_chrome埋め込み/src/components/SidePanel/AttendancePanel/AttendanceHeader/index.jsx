...
export default function AttendanceHeader({
  ...
  const selectedFacilityNames = useMemo(() => {
    return attendanceFacilities
      .filter((facility) => Boolean(attendanceFacilityMap[String(facility.facility_id)]))
      .map((facility) => facility.name)
  }, [attendanceFacilities, attendanceFacilityMap])
  ...
  const fetchBlockHint = attendanceFacilitiesLoading
    ? '施設を取得中...'
    : !canFetchAttendance
      ? '施設の取得完了後に一覧を取得できます'
      : ''
