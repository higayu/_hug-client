(() => {
  ...
  */
  ...
  const shouldRunScheduledUpdate = () => {
    if (
      typeof window.HugAttendance.isAutoUpdatePaused === "function" &&
      window.HugAttendance.isAutoUpdatePaused()
    ) {
      return false;
    }
    if (document.hidden && !window.HugAttendance?.isSidePanelHost) {
      return false;
    }
    return true;
  };
  ...
  /*
    ...
    options.force === true のときは停止中・非表示タブでも実行（手動更新など）
  */
  const runAttendanceUpdate = async (options = {}) => {
  ...
  /*
    ページ読み込み後に開始（サイドパネルでは load が既に発火済みのことがある）
  */
  if (document.readyState === "complete") {
    startTimerLoop();
  } else {
    window.addEventListener("load", startTimerLoop);
  }
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    const elapsed = Date.now() - lastScheduledRunAt;
    if (lastScheduledRunAt === 0 || elapsed >= INTERVAL_MS) {
      runAttendanceUpdate();
    }
  });
