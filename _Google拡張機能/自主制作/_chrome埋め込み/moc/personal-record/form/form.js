(() => {
  ...
  const initPanelContent = async () => {
    const panel =
      document.getElementById("hug-personal-record-form") ||
      document.querySelector("#hug-personal-record-form.hug-sidepanel-form-root");
    if (panel?.dataset.hugContentWired === "1") {
      return;
    }

    const bodyEl =
      document.getElementById("hug-pr-panel-body") ||
      document.querySelector(
        "#hug-personal-record-form.hug-sidepanel-form-root"
      );
    if (!bodyEl) {
      return;
    }
