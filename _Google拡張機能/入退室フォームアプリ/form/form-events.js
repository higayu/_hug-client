(() => {
    ...
    if (action === "enter") {
      ...
      try {
        ...
        if (
          ...
          await window.HugAttendance.runAttendanceUpdate({ force: true });
        }
      } catch (e) {
  ...
  const RECORD_PROCEEDINGS_BASE =
    "https://www.hug-ayumu.link/hug/wm/record_proceedings.php";

  const CONTACT_BOOK_BASE =
    "https://www.hug-ayumu.link/hug/wm/contact_book.php";

  const onRecordProceedingsMove = (event) => {
    const btn = event.target.closest(".hug-btn-record-move");
    if (!btn) return;

    const cId = String(btn.getAttribute("data-hug-cid") || "").trim();
    if (!cId) return;

    event.preventDefault();
    const url =
      `${RECORD_PROCEEDINGS_BASE}?mode=edit&select_child=${encodeURIComponent(cId)}`;
    window.open(url, "_blank", "noopener,noreferrer");
  };

  const onPersonalRecordOpen = (event) => {
    const btn = event.target.closest(".hug-btn-personal-record");
    if (!btn) return;
    ...
    const calDate = String(btn.getAttribute("data-hug-cal-date") || "").trim();
    const q = new URLSearchParams({
      id: cId,
      hug_auto_personal: "1"
    });
    if (calDate) {
      q.set("hug_cal_date", calDate);
    }

    window.open(
      `${CONTACT_BOOK_BASE}?${q.toString()}`,
      "_blank",
      "noopener,noreferrer"
    );
  };
  ...
  Object.assign(Form, {
    onHugPostClick,
    onRecordProceedingsMove,
    onPersonalRecordOpen,
    onRefreshClick,
    onAlertPrefChange,
    onHalfTimePersist,
