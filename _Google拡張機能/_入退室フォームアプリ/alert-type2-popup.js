...
(() => {
  ...
  const buildPopupFeatures = () => {
    ...
    const left = availLeft + Math.max(0, Math.round((availWidth - POPUP_WIDTH) / 2));
    const top = availTop + 16;
  ...
  const openAlertType2Popup = (opt) => {
    ...
    let popup;
    try {
      popup = window.open("about:blank", POPUP_WINDOW_NAME, buildPopupFeatures());
    } catch (e) {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .sub { margin: 0 0 6px; font-size: 11px; color: #ffc9c9; }
    .note {
      margin: 0 0 8px;
      padding: 6px 8px;
...
<body>
  ...
  <p class="note">経過時間超過で入室中です。パネルで退室記録を確認してください。</p>
  <table>
    <thead>
      <tr>
        <th>氏名</th>
  ...
  <p class="foot">パネル更新で内容は上書きされます。</p>
</body>
</html>`;
