...
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='AI校正・文章生成実行ログ';

CREATE TABLE IF NOT EXISTS `ai_prompts` (
  `prompt_id` int(11) NOT NULL AUTO_INCREMENT,
  `staff_id` int(11) DEFAULT NULL,
  `item_id` tinyint(4) unsigned DEFAULT NULL,
  ...
  UNIQUE KEY `unique_active_staff_item` (`is_active`,`staff_id`,`item_id`),
  KEY `idx_ai_prompts_updated_by` (`updated_by`),
  KEY `idx_ai_prompts_updated_at` (`updated_at`),
  KEY `FK_ai_prompts_staffs` (`staff_id`),
  KEY `FK_ai_prompts_m_pronpt_items` (`item_id`),
  CONSTRAINT `FK_ai_prompts_m_pronpt_items` FOREIGN KEY (`item_id`) REFERENCES `m_pronpt_items` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_ai_prompts_staffs` FOREIGN KEY (`staff_id`) REFERENCES `staffs` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_ai_prompts_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `staffs` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='AIプロンプトの最新状態';
...
DELIMITER //
CREATE PROCEDURE `register_manager_assignment`(
    IN `p_children_id` INT,
    IN `p_children_name` VARCHAR(255),
    IN `p_notes` TEXT,
    IN `p_notes2` TEXT,
    IN `p_personal_tmp` TEXT,
    IN `p_pronunciation_id` INT,
    IN `p_children_type_id` INT,
    IN `p_is_delete` TINYINT,
    IN `p_leaving_at` DATE,
    IN `p_facility_id` INT,
    IN `p_staff_id` INT,
    IN `p_day_of_week_id` TINYINT,
    IN `p_priority` TINYINT,
    IN `p_support_start_time` TIME,
    IN `p_support_end_time` TIME
)
BEGIN
    DECLARE `v_children_exists` INT DEFAULT 0;
    ...
    /* 必須値・値域チェック */
    IF p_children_id IS NULL OR p_children_id <= 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'children_idが不正です。';
    END IF;

    IF p_facility_id IS NULL OR p_facility_id <= 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'facility_idが不正です。';
    END IF;

    IF p_staff_id IS NULL OR p_staff_id <= 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'staff_idが不正です。';
    END IF;

    IF p_day_of_week_id IS NULL OR p_day_of_week_id <= 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'day_of_week_idが不正です。';
    END IF;

    IF p_children_type_id IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'children_type_idが指定されていません。';
    END IF;

    IF COALESCE(p_is_delete, 0) NOT IN (0, 1) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'is_deleteは0または1で指定してください。';
    END IF;

    IF COALESCE(p_priority, 0) NOT IN (0, 1) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'priorityは0または1で指定してください。';
    END IF;

    IF p_support_start_time IS NOT NULL
       AND p_support_end_time IS NOT NULL
       AND p_support_start_time >= p_support_end_time THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '支援終了時間は支援開始時間より後にしてください。';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定された施設がfacilitysテーブルに存在しません。';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定された職員がstaffsテーブルに存在しません。';
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM `day_of_week`
        WHERE `id` = p_day_of_week_id
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定された曜日がday_of_weekテーブルに存在しません。';
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM `children_type`
        WHERE `id` = p_children_type_id
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定されたchildren_type_idがchildren_typeテーブルに存在しません。';
    END IF;

    IF p_pronunciation_id IS NOT NULL
       AND NOT EXISTS (
            SELECT 1
            FROM `pronunciation`
            WHERE `id` = p_pronunciation_id
       ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定されたpronunciation_idがpronunciationテーブルに存在しません。';
    END IF;
    ...
    SELECT COUNT(*)
      ...
      FROM `children`
     WHERE `id` = p_children_id;

    /* 未登録児童だけ追加。既存児童の基本情報は担当登録では上書きしない。 */
    IF v_children_exists = 0 THEN
        INSERT INTO `children` (
            `id`,
            `name`,
            `furigana`,
        ...
        VALUES (
            ...
            p_notes2,
            p_personal_tmp,
            COALESCE(p_is_delete, 0),
            p_leaving_at
        );
    END IF;

    /* 施設と児童の紐付けを追加。既存なら何もしない。 */
    ...
    ON DUPLICATE KEY UPDATE
        ...
        `children_id` = VALUES(`children_id`);
    ...
    VALUES (
        ...
        p_support_start_time,
        p_support_end_time
    )
    ON DUPLICATE KEY UPDATE
        `priority` = VALUES(`priority`),
        `support_start_time` = VALUES(`support_start_time`),
        `support_end_time` = VALUES(`support_end_time`);
    ...
    SELECT
        ...
        m.`day_of_week_id`,
        m.`priority`,
        m.`support_start_time`,
        m.`support_end_time`
    FROM `managers2` AS m
    INNER JOIN `children` AS c
        ON c.`id` = m.`children_id`
    WHERE m.`children_id` = p_children_id
      AND m.`facility_id` = p_facility_id
      AND m.`staff_id` = p_staff_id
      AND m.`day_of_week_id` = p_day_of_week_id
    LIMIT 1;
END//
DELIMITER ;
...
) ENGINE=InnoDB AUTO_INCREMENT=67 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='サービス全体記録';
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = '指定された児童がchildrenテーブルに存在しません。';
    END IF;
